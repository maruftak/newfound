...
`newfound` is a continuous attack-surface change monitor for bug-bounty hunters and
...
`newfound` closes that gap: it snapshots your surface on a schedule, computes the
...
> ⚠️ **Authorized use only.** Point `newfound` at assets you own or domains that are
...
`newfound` orchestrates battle-tested tools instead of reinventing recon. Its value is
the **diff + prioritization + alerting** layer on top.

## Install

`newfound` is a single Go binary. It shells out to [`subfinder`][sf] and [`httpx`][hx]
(from ProjectDiscovery) for discovery and probing — install those too.

```bash
# newfound
go install github.com/maruftak/newfound/cmd/newfound@latest
...
```bash
git clone https://github.com/maruftak/newfound
cd newfound
go build -o newfound ./cmd/newfound
```
